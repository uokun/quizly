...
> 8. **知識の永続化**: 仕様は `docs/antigravity/` フォルダ内の `_CONTEXT.md` にも反映すること。
...
3.  仕様や設計に影響がある場合の `docs/antigravity` への知識同期 (Knowledge Sync)

## 手順

### 1. 前提知識の確認 (Context Loading)

タスクを開始する前に、まず `docs/antigravity` フォルダ内の関連する `_CONTEXT.md` ファイルを読み込み、プロジェクトの文脈や既存の設計方針を確認してください。
...
### 6. docs/antigravity への同期 (Knowledge Sync)

タスク完了後、以下の手順で情報を永続化してください。

1.  **対象ファイルの選定**: `docs/antigravity` フォルダ内を検索し、今回の変更に関連するファイル（例: `main/index_CONTEXT.md`, `pos/mobile-order_status_CONTEXT.md` 等）を特定してください。適切なファイルがない場合は新規作成してください。
2.  **情報の追記・更新**: 特定したファイルに対し、以下の内容を追記・更新してください。
    ...
    ※ `task.md` や `implementation_plan.md` はタスク完了後に参照されなくなる可能性があるため、**将来的に必要な情報は全て `docs/antigravity` 配下に移動**させてください。

3.  **完了報告**: ユーザーへの完了報告時に、「どの情報をどのファイル(`docs/antigravity/xxx.md`)に記録したか」を明記してください。
...
なお、`docs/antigravity/` 配下の `_CONTEXT.md` への同期も必要に応じて行いますが、**デバッグ知見や「なぜそうしたか」の全履歴は `CHANGELOG.md` を正として集約**させてください。
